package cn.oneplustow.api.user.aspect;

import cn.hutool.core.util.StrUtil;
import cn.hutool.json.JSONUtil;
import cn.oneplustow.api.user.async.RecordOperTimerTask;
...
public class LogAspect
{
    ...
    // 配置织入点
    @Pointcut("@annotation(cn.oneplustow.common.annoatation.Log)")
    ...
    protected void handleLog(final JoinPoint joinPoint, final Exception e, Object jsonResult)
    {
        try
        {
            ...
            // 获取当前的用户
            //LoginUser loginUser = SpringUtil.getBean(TokenService.class).getLoginUser(ServletUtils.getRequest());
...
//            }
            ...
            // 保存数据库
            AsyncManager.me().execute(new RecordOperTimerTask(operLog));
        }
        catch (Exception exp)
        {
            // 记录本地异常日志
     ...
     * @throws Exception
     */
    public void getControllerMethodDescription(JoinPoint joinPoint, Log log, SysOperLogModel operLog) throws Exception
    {
        // 设置action动作
